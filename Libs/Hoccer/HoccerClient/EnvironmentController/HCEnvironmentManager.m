...
@property (retain) CLLocation *overwriteLocation;
...
- (HCEnvironment *)environment {
    if (!overwriteLocation){
        ...
        HCEnvironment *location = [[HCEnvironment alloc]
                                   initWithLocation:self.overwriteLocation bssids:[WifiScanner sharedScanner].bssids];
    location.hoccability = hoccability;
      return [location autorelease];
...
- (BOOL)hasLocation {
	return (locationManager.location != nil || self.overwriteLocation != nil);
}
...
- (void)overwriteLocationWithCoordinate:(CLLocationCoordinate2D)coordinate{
    self.overwriteLocation = [[CLLocation alloc] initWithCoordinate:coordinate altitude:0 horizontalAccuracy:200 verticalAccuracy:200 timestamp:[NSDate date]];
    [self updateHoccability];
}
